	...
	for (int i = 1; i < 65; i++)
	{
		...
		if (SETTINGS::settings.aim_bool)
		{
			...
			if (accepted_inaccuracy(weapon) > SETTINGS::settings.chance_val) //50
			{
				...
				{
					if (autowall->CalculateDamage(local_position, head_point, local_player, entity).damage > SETTINGS::settings.damage_val && can_shoot(cmd)) //targetp, 15
					{
				...
				{
					if (autowall->CalculateDamage(local_position, scan_point, local_player, entity).damage > SETTINGS::settings.damage_val && can_shoot(cmd)) //targetp, 15
					{
						if (SETTINGS::settings.reverse_bool)
							GLOBAL::should_send_packet = false;
